...
class ScrappyKNN():
	# fit dataset into out class
	def fit(self, X_train, y_train):
		self.X_train = X_train
		self.y_train = y_train

	# predict random
	def predict_random(self, X_test):
		predictions = []
		for row in X_test:
	...
	# calculate nearest neighbors using euclidean distance between two points
...
# predict the target on given test data using SKNN random
scrappy_predictions_random = sknn_classifier.predict_random(X_test)
...
print("Accuracy Score using our Random Scrappy KNN: %s" % accuracy_score(y_test, scrappy_predictions_random))
print("Accuracy Score using our Euclidean Scrappy KNN: %s" % accuracy_score(y_test, scrappy_predictions))
